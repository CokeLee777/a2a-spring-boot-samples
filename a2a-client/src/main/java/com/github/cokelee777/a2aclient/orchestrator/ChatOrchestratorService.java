...
public class ChatOrchestratorService {
    ...
    public String handleUserQuery(String userMessage) {
        ...
        if (analysis.needsOrderAgent() && analysis.orderNumber() != null) {
            String orderResult = agentInvokerService.callOrderAgent(analysis.orderNumber() + " 주문 취소해줘");
            response.append(orderResult);
        }
        if (analysis.needsDeliveryAgent() && analysis.trackingNumber() != null) {
            if (!response.isEmpty()) response.append("\n\n---\n\n");
            String deliveryResult = agentInvokerService.callDeliveryAgent(analysis.trackingNumber() + " 배송 조회해줘");
            response.append(deliveryResult);
        }
